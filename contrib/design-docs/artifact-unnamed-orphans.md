...
Two options are proposed below. Both share a common cleanup for legacy orphans (Change C1).

## **How ORAS handles OCI artifacts**

[ORAS (OCI Registry As Storage)](https://oras.land/docs/concepts/artifact/) defines OCI artifacts as content-addressable objects that can be referenced by a **tag** or a **digest**. Tags are mutable pointers that can move between digests; digests are immutable content hashes. When a tag is pushed to a new digest, the old digest remains in the registry and is still accessible by its `repo@sha256:...` reference. Cleanup of untagged content is left to garbage collection or explicit user action.

This model is consistent with how container image registries and `podman images` work: pulling a new `latest` does not delete the old image, it just moves the tag. The old image becomes "dangling" (no tag) but is still listed and removable by digest.
...
Error when pulling an artifact that already exists locally. Add a `--replace` flag to explicitly delete the old artifact before pulling, consistent with `podman artifact add --replace`.
...
**Pros:**
- No dangling artifacts, store stays clean.
- Consistent with `podman artifact add --replace`.
- Simple implementation.

**Cons:**
- Breaks users who script `podman artifact pull` without `--replace`.
- Not consistent with `podman pull` (images), which silently replaces.
...
Silently retag old entries with a digest reference after pull, preserving the repository name. This is consistent with how `podman pull` handles images and how ORAS handles OCI artifacts.
...
**Pros:**
- Consistent with `podman pull` (images) and ORAS.
...
With Option B, old artifact digests accumulate over time (same as images). A future `podman artifact prune` command should be added to remove untagged artifacts collectively, analogous to `podman image prune`. This is not part of this change.
...
**Option A:** A new `--replace` flag is added to `podman artifact pull`. Without it, re-pulling an existing artifact errors. This is consistent with `podman artifact add --replace`.

**Option B:** No new flags. `podman artifact pull` silently retags old entries when the tag moves to a new digest. `podman artifact ls` shows old artifacts with repository name and empty tag. `podman artifact rm` works by digest prefix. This is consistent with `podman pull` (images).
