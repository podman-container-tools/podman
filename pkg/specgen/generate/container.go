...
func CompleteSpec(ctx context.Context, r *libpod.Runtime, s *specgen.SpecGenerator) ([]string, error) {
	...
	if s.LogConfiguration != nil && s.LogConfiguration.Options != nil {
		if labelOpts, ok := s.LogConfiguration.Options["labels"]; ok {
			for _, l := range strings.Split(labelOpts, ",") {
				l = strings.TrimSpace(l)
				if v, ok := s.Labels[l]; ok {
					if k := specgenutil.SanitizeLogKey(l); k != "" {
						if s.LogConfiguration.Labels == nil {
							s.LogConfiguration.Labels = make(map[string]string)
						}
						s.LogConfiguration.Labels[k] = v
					}
				}
			}
			delete(s.LogConfiguration.Options, "labels")
		}
		if envOpts, ok := s.LogConfiguration.Options["env"]; ok {
			for _, e := range strings.Split(envOpts, ",") {
				e = strings.TrimSpace(e)
				if v, ok := s.Env[e]; ok {
					if k := specgenutil.SanitizeLogKey(e); k != "" {
						if s.LogConfiguration.Labels == nil {
							s.LogConfiguration.Labels = make(map[string]string)
						}
						s.LogConfiguration.Labels[k] = v
			...
			delete(s.LogConfiguration.Options, "env")
		}
	}
