...
var _ = Describe("Podman logs", func() {
	...
	It("using journald log-opt labels to promote container labels", func() {
		SkipIfJournaldUnavailable()
		SkipIfConmonVersionLessThan("2.2.0")
		containerName := "journal-labels-promote"
		logc := podmanTest.Podman([]string{
			"run", "--log-driver", "journald",
			"--label", "app_name=myapp",
			"--log-opt", "labels=app_name",
			"-d", "--name", containerName,
			ALPINE, "sh", "-c", "echo podman; sleep 0.1",
		})
		logc.WaitWithDefaultTimeout()
		Expect(logc).To(ExitCleanly())
		cid := logc.OutputToString()
		...
		Eventually(func(g Gomega) {
			cmd := exec.Command("journalctl", "--no-pager", "-o", "json", "--output-fields=APP_NAME", fmt.Sprintf("CONTAINER_ID_FULL=%s", cid))
			out, err := cmd.CombinedOutput()
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(string(out)).To(ContainSubstring("myapp"))
		}).Should(Succeed())
	})

	It("using journald log-opt env to promote env vars", func() {
		SkipIfJournaldUnavailable()
		SkipIfConmonVersionLessThan("2.2.0")
		containerName := "journal-env-promote"
		logc := podmanTest.Podman([]string{
			"run", "--log-driver", "journald",
			"-e", "APP_VERSION=1.2.3",
		...
		Eventually(func(g Gomega) {
			cmd := exec.Command("journalctl", "--no-pager", "-o", "json", "--output-fields=APP_VERSION", fmt.Sprintf("CONTAINER_ID_FULL=%s", cid))
			out, err := cmd.CombinedOutput()
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(string(out)).To(ContainSubstring("1.2.3"))
		}).Should(Succeed())
	})
